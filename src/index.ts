...
import "./forEachAsync";
import {
  IBingImageFeed as IBingImageOfTheDayFeed,
  ImagesEntity,
} from "./bingImageFeed.interface";
...
consola.log(
  `Welcome to ${gradient(["#f26522", "#8dc63f", "#00aeef", "#ffc20e"])(
    "-=<[ Bing Image-Of-The-Day Video Generator ]>=-"
  )} ${options.version}`
);
...
async function downloadImage(imageElement: ImagesEntity, locale: string) {
...
async function fetchBingImageOfTheDayFeedJSON(
  feedUrl
): Promise<IBingImageOfTheDayFeed> {
